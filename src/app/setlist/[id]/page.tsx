...
export default function SetlistReviewPage() {
  ...
  function moveItem(index: number, direction: -1 | 1) {
    ...
    setItems(updated);
    reorderSetlistItems(id, updated.map(i => i.id));
  }
  ...
  return (
    <div className="p-4 overflow-y-auto h-screen">
      ...
      {editMode && (
        <p className="text-sm text-orange-600 bg-orange-50 border border-orange-200 rounded px-3 py-2 mb-4">
          Tap ↑ or ↓ to move a song. Tap ✕ to remove it. Press <strong>Done</strong> when finished.
        </p>
      )}
                ...
                <tr key={item.id} className="hover:bg-gray-50">
                  <td>
                    <div className="flex gap-1">
                      <button
                        ...
                        disabled={index === 0}
                        className="px-2 py-1 text-lg bg-gray-100 hover:bg-gray-200 rounded disabled:opacity-20"
                      >↑</button>
                      <button
                        onClick={() => moveItem(index, 1)}
                  ...
                  <td className="font-medium">
                    ...
                    {item.matched_display_title && (
                      <span className="text-xs text-gray-400 ml-2">→ {item.matched_display_title}</span>
                    )}
                  </td>
                  <td className="text-center">
                    <button
                      onClick={() => handleRemoveItem(item.id)}
                      className="text-red-500 hover:text-red-700 bg-transparent text-lg px-2"
                    >✕</button>
                  </td>
                </tr>
              ) : (
      ...
      {/* Add song panel — hidden in edit mode */}
      {!editMode && (
